...
def pitch_to_key(pitch: str):
    pox_x = {"C": 0, "D": 1, "E": 2, "F": 3, "G": 4, "A": 5, "B": 6}[pitch[0]]
    pox_y = 0

    if pitch[1].isdigit():
        pox_x += (int(pitch[1]) - 4) * 7
    elif pitch[1] == "#":
        if pitch[2] == "#":
            pox_x += (int(pitch[3]) - 4) * 7 + 1
        else:
            pox_y = 1
            pox_x += (int(pitch[2]) - 4) * 7
    elif pitch[1] == "b" or pitch[1] == "-":
        if pitch[2] == "b" or pitch[2] == "-":
            pox_x += (int(pitch[3]) - 4) * 7
        else:
            pox_y = 1
            pox_x += (int(pitch[2]) - 4) * 7 - 1

    return (pox_x, pox_y)


def note_to_diff(fingering_data):
...
def count_fingering(fingering_data):
    hidden_state = list(
        zip(
